...
- (UIView *)lw_findSuperViewWithClass:(Class)clazz {
    if (self == nil) {
        ...
        return [self.superview lw_findSuperViewWithClass:clazz];
    }
}
...
- (UIView *)lw_findFirstResponder {
    ...
    for (UIView *v in self.subviews) {
        UIView *fv = [v lw_findFirstResponder];
        if (fv) {
            return fv;
        }
    }
...
- (NSArray *)lw_allSubviews {
    ...
    for (UIView *view in self.subviews) {
        [array addObjectsFromArray:[view lw_allSubviews]];
    }
...
+ (instancetype)lw_loadViewFromNib {
    return [self lw_loadViewFromNibWithName:NSStringFromClass([self class])];
}

+ (instancetype)lw_loadViewFromNibWithName:(NSString *)nibName {
    return [self lw_loadViewFromNibWithName:nibName owner:nil];
}

+ (instancetype)lw_loadViewFromNibWithName:(NSString *)nibName owner:(id)owner {
    return [self lw_loadViewFromNibWithName:nibName owner:owner bundle:[NSBundle mainBundle]];
}
